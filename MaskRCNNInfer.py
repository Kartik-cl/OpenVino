...
class Inference:
    def __init__(self):
        ...
        [
            Resize(max_size=self.args.fit_max_image_size, window_size=self.args.fit_window_size, size=self.args.size),
            ToTensor(),
            Normalize(mean=self.args.mean_pixel, std=[1., 1., 1.], rgb=self.args.rgb),
        ]
    )
        self.dataset = get_dataset(self.args.dataset, False, False, self.transforms)
        self.batch_size = 1
        self.net = MaskRCNNOpenVINO(self.args.openvino_model_path, self.args.checkpoint_file_path,
                            device=self.args.device, plugin_dir=self.args.plugin_dir,
                            cpu_extension_lib_path=self.args.cpu_extension,
                            collect_perf_counters=self.args.show_performance_counters)
    ...
    def output(self, img_path):
        ...
        self.args.show_boxes = True #need to pass as a parameter

        #viz = Visualizer(self.dataset.classes, confidence_threshold=self.args.prob_threshold,
        #                show_boxes=self.args.show_boxes, show_scores=self.args.show_scores)
        ...
        demo_dataset = ImagesDataset(img_path, labels=self.dataset.classes, transforms=self.transforms)
        num_workers = 1
        tracker = None
        ...
        for data_batch in tqdm(iter(data_loader), total=frames_num if frames_num != sys.maxsize else 0):
            ...
            with torch.no_grad(), inference_timer:
                boxes, classes, scores, _, masks = self.net(im_data, im_info)
                image = data_batch['original_image'][0]
                image = cv2.resize(image, (1344,768), interpolation = cv2.INTER_AREA)
                #cv2.rectangle(image, (boxes[1][0], boxes[1][1]), (boxes[1][2], boxes[1][3]), (0,0,255), 4)
            ...
            scores, classes, boxes, masks = postprocess(scores, classes, boxes, masks,
                                                        ...
                                                        confidence_threshold=self.args.prob_threshold)
            ...
            vertices = []
            for mask in masks:
                mask_indexes = np.stack(np.nonzero(masks[0]), axis=-1)
                mask_indexes = list(np.flip(mask_indexes, axis=1))
                mask_indexes = [l.tolist() for l in mask_indexes]
                vertices.append(mask_indexes)
            scores = list(scores)
            scores =  [l.tolist() for l in scores]
            classes = [l.tolist() for l in classes]
            timer.tic()
            return vertices, scores, classes

        if inference_timer.average_time > 0:
            logging.info('Average inference FPS: {:3.2f}'.format(1 / inference_timer.average_time))


    def openVino(self, rgb_image):
        cv2.imwrite("temp1.png", rgb_image)
        vertices, scores, predictions = self.output("temp1.png")
        sampled_vertices = []
        for i in range(len(vertices)):
            sample_points = []
            for j in range(len(vertices[i])):
                if(j%200 == 0):
                    sample_points.append(vertices[i][j]) #place holder, will solve this geometrically
            sampled_vertices.append(sample_points)
        print(vertices)
        input("here")
        return_dict = {"vertices" : sampled_vertices, "scores" : scores, "predictions": predictions} #send vertices in later version
        return return_dict
