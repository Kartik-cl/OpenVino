...
def init(args):
    ...
    return net, dataset, transforms, batch_size


def output(net, args, dataset, transforms, batch_size, img_path):
    
    args.show_boxes = True #need to pass as a parameter
    ...
    demo_dataset = ImagesDataset(img_path, labels=dataset.classes, transforms=transforms)
    num_workers = 1
    tracker = None
...
def openvino(rgb_image):
    args = parse_args()
    net, dataset, transforms, batch_size = init(args)
    vertices, scores, predictions = output(net, args, dataset, transforms, batch_size, "temp.png")
    return_dict = {"vertices" : vertices, "scores" : scores, "predictions": predictions}
    return return_dict
...
if __name__ == '__main__':
    args = parse_args()
    net, dataset, transforms, batch_size = init(args)
    vertices, scores, predictions = main(net, args, dataset, transforms, batch_size, "temp.png")
    return_dict = {"vertices" : vertices, "scores" : scores, "predictions": predictions}
